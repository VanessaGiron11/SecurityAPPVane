...
import Grupo3A.persistencia.UserDAO; // Importa la interfaz o clase UserDAO, que define las operaciones de acceso a datos para la entidad User.
...
import Grupo3A.Utils.CUD; // Importa el enum CUD (Create, Update, Delete),  para indicar el tipo de operación que se está realizando (Crear, Actualizar, Eliminar).

import javax.swing.*; // Importa el paquete Swing, que proporciona clases para crear interfaces gráficas de usuario (GUIs).

import Grupo3A.dominio.User; // Importa la clase User, que representa la entidad de usuario en el dominio de la aplicación.
...
public class UserWriteForm extends JDialog {
    ...
    private void initCBStatus() {
        ...
        DefaultComboBoxModel<CBOption> model = (DefaultComboBoxModel<CBOption>) cbEstatus.getModel();
    ...
    private void setValuesControls(User user) {
        ...
        cbEstatus.setSelectedItem(new CBOption(null, user.getStatus()));
        ...
        if (this.cud == CUD.CREATE) {
            ...
            cbEstatus.setSelectedItem(new CBOption(null, 1));
        }
    ...
    private boolean getValuesControls() {
        ...
        CBOption selectedOption = (CBOption) cbEstatus.getSelectedItem();
        ...
        this.en.setName(textName.getText());
        // Establece el correo electrónico del usuario.
        this.en.setEmail(textEmail.getText());
